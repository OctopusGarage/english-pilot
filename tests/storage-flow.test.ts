...
describe('local learning storage flow', () => {
  ...
  it('updates a learning item in the review queue', () => {
    const original = '参考~/programming/OctopusGarage/telegram-bridge 一样对齐处理';
    ...
    const updated = runCli([
      ...
      item.id,
      '--suggested',
      'Align this implementation with ~/programming/OctopusGarage/telegram-bridge.',
      '--scene',
    ...
    expect(reviewItem.suggested).toBe('Align this implementation with ~/programming/OctopusGarage/telegram-bridge.');
  });
  ...
  it('previews and confirms cleanup of likely noisy review items', () => {
    const noisy = recordLearningItem({
      original: [
        'You are a spec compliance reviewer for Task 5 only. Do not edit files.',
        'Review the implementation in this worktree: /Users/kingsonwu/project',
        'Spec to check:',

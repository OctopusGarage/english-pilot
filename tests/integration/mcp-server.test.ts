...
describe('MCP tools', () => {
  ...
  it('lists the first-version MCP tool names', () => {
    expect(listMcpTools()).toEqual([
      ...
      'english_daily_check',
      'english_daily_pack',
      'english_input_history',
      'english_notes_history',
      'english_learning_brief',
      'english_mark_review',
  ...
  it('returns input history for agent-side learning summaries', () => {
    runCli(['config', 'use', 'coach']);
    runCli(['check', '--text', '我想创建一个 new project，用来辅助英语学习。', '--json']);
    runCli(['check', '--text', 'what is the weather about 广州', '--json']);

    const result = handleMcpToolCall('english_input_history', {
  ...
  it('returns English notes history for agent-side review reuse', () => {
    handleMcpToolCall('english_record_learning_item', {
      ...
      tags: ['weather', 'speaking'],
    });

    const result = handleMcpToolCall('english_notes_history', {
      tag: 'weather',
      limit: 5,
    });
  ...
  it('builds an agent-ready English learning brief from inputs and notes', () => {
    runCli(['config', 'use', 'coach']);
    runCli(['check', '--text', '我想创建一个 new project，用来辅助英语学习。', '--json']);
    handleMcpToolCall('english_record_learning_item', {
      original: 'what is the weather about 广州',
    ...
    const result = handleMcpToolCall('english_learning_brief', {
      date: new Date().toISOString().slice(0, 10),
      limit: 10,
    });
    ...
    expect(result).toMatchObject({
      ...
      notes: expect.arrayContaining([
        expect.objectContaining({
          suggested: expect.stringContaining('new project'),
        }),
        expect.objectContaining({
          suggested: 'What is the weather like in Guangzhou?',
        }),
      ]),
      teachingPrompt: expect.stringContaining('Use this EnglishPilot learning brief'),
      suggestedActivities: expect.arrayContaining([
        expect.stringContaining('daily recap'),
        expect.stringContaining('short English speech'),
      ]),
    });
  });
  ...
  it('updates review items through MCP', () => {
    const recorded = handleMcpToolCall('english_record_learning_item', {
      original: '参考~/work/reference-service 一样对齐处理',
      suggested: '~/work/reference-service.',
    }) as { item: { id: string } };

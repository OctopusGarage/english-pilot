...
`integrations account-validate --target wechat` runs the executable validation playbook: preflight, dry-run, send-readiness, and final network-send stage. Direct HTTP/request-preview sending is deprecated and blocked, so the network-send stage reports the long-connection blocker instead of sending. Add `--record` to persist a sanitized validation summary.
...
`integrations deliver --target feishu [--date YYYY-MM-DD]` sends a compact daily review through the local `tmux-claude-bot` Feishu/Lark notifier. Before real delivery, it removes low-quality records and never-reviewed records that have been overdue for at least 60 days; reviewed records are protected from age-based cleanup. Preview and dry-run commands never perform cleanup. Admission also rejects empty, unchanged, trivial, generic, task-like, or oversized learning items while preserving intentional vocabulary captures.

The delivery selects up to 12 due items with 4 target slots each for newly due items from the last 14 days, previously reviewed items, and the older never-reviewed backlog. Empty cohort slots spill into the other cohorts. The backlog uses a deterministic date-based rotation, so retries on the same date are stable while consecutive days expose different records. Delivery does not mark an item as learned or change its spaced-repetition schedule; only explicit `again`, `hard`, or `easy` outcomes do that. Messages are capped at about 1,000 characters and append `[truncated; see the linked report/logs for full details]` when necessary. The project-bound `english-pilot` tmux session is used by default; set `ENGLISH_PILOT_FEISHU_SESSION` to override group routing.
